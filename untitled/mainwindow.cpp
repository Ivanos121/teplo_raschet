...
void MainWindow::on_action_2_triggered()
{
    ...
    QString filters("CSV files (*.csv);;All files (*.*)");
    QString defaultFilter("CSV files (*.csv)");
    QString fileName = QFileDialog::getOpenFileName(this,"Open file", QCoreApplication::applicationDirPath(),
                                                       filters, &defaultFilter);
    QFile file(fileName);
    ...
    if(file.open(QFile::ReadOnly))
    {
        ...
        while (!textStream.atEnd())
            rowNumbers << textStream.readLine();

         file.close();
    }

    for(int x=0;x<rowNumbers.size()-1;x++)
    {
        qDebug() << Qt::endl;
         columnNumbers=rowNumbers.at(x + 1).split(";");
         for(int y=0;y<columnNumbers.size();y++)
         {
            ui->tableWidget->item(x,y)->setText(columnNumbers[y]);
            qDebug() << Qt::endl << y;
         }
    }
}
